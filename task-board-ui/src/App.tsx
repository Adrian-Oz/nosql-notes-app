...
function App() {
  ...
  useEffect(() => {
    ...
    async function strategyRun() {
      ...
      try {
        ...
        await strategy.load({ hydrateBoards, createBoard, getBoards });
        if (!isCurrent) return;
      } catch (error) {
  ...
  useEffect(() => {
    const unsubscribe = useBoardStore.subscribe((state) => {
      ...
      const strategy = getStrategy(mode);
      // const userId = useAuthStore.getState().user?.uid;
      strategy.save({ hydrateBoards, createBoard, getBoards, });
    });
